...
- `src/content/` &mdash; תוכן שנערך בקוד (שיעורים, עדכונים, אירועים, רפואות)
  &mdash; ראו "איך מוסיפים תוכן" למטה.
- `src/lib/` &mdash; API של Hebcal לזמני שבת/חגים, ושליפת CSV מ-Google Sheets (אזכרות).
...
## אזכרות — Google Form, לא קובצי תוכן

בניגוד לשאר התוכן, אזכרות **לא** נערכות כקבצים בריפו. חברי הקהילה נרשמים
בעצמם דרך Google Form (הכפתור "רישום אזכרה חדשה" בעמוד `/memorials`
מקושר אליו), התשובות נכנסות לגיליון מקושר, והעמוד שולף אותן live מה-CSV
המפורסם של הגיליון (`MEMORIALS_SHEET_CSV_URL` ב-`src/data/site.ts`). זה
נבחר במקום ניתוב ידני דרך הגבאי מכיוון שרישום ידני לא הניב נתונים מדויקים
&mdash; כל חבר קהילה יכול להוסיף ישירות.

## איך מוסיפים תוכן (עדכונים, אירועים, רפואות, שיעורים)
